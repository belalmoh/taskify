...
export const Card = ({
  ...
  return (
    <div className="group relative flex flex-col gap-3 rounded-[3px] bg-card p-3 shadow-card transition-all hover:bg-gray-50 hover:shadow-card-hover cursor-pointer border border-transparent hover:border-primary/20">
        ...
        {assignees.length > 0 && (
          <div className="flex -space-x-1.5 overflow-hidden">
            {assignees.map((assignee, i) => (
              <Avatar key={i} src={assignee.src} fallback={assignee.fallback} size="sm" className="ring-2 ring-white" />
            ))}
          </div>
        )}
      </div>
